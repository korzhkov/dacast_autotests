...
const ownerVodId = process.env._VOD_ID;
...
if (!ownerApiKey || !ownerVodId || !otherApiKey || !otherVodId || !hostAPI) {
  ...
  // API key check
  if (!ownerApiKey) missingVars.push('_API_KEY');
  if (!ownerVodId) missingVars.push('_VOD_ID');
...
test('POST VOD (1): should initiate and upload VOD successfully with correct credentials (200)', async () => {
  ...
  // POST: INITIATE UPLOAD (POST /v2/videos)
  ...
  // POST: EXECUTE FILE UPLOAD (exec uploadCurlCmd)
  if (uploadCurlCmd) {
