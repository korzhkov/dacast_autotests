...
// Note, Expo UI loads kind of slow.
  ...
  await test.step('Drag and drop second video to Expo', async () => {
    ...
    // Add simplified visual indicators
    await page.evaluate(({ sourceCenter, targetBoundingBox }) => {
      const createIndicator = (id, x, y, color) => {
        const indicator = document.createElement('div');
        indicator.id = id;
      ...
      createIndicator('source-indicator', sourceCenter.x, sourceCenter.y, 'red');
      createIndicator('target-indicator', targetBoundingBox.x + (targetBoundingBox.width * 0.25), targetBoundingBox.y + targetBoundingBox.height + 10, 'green');
      createIndicator('mouse-indicator', sourceCenter.x, sourceCenter.y, 'blue');
    }, { sourceCenter, targetBoundingBox });

    // Calculate coordinates for the drop target (below the image in the left part of the div)
    const targetX = targetBoundingBox.x + (targetBoundingBox.width * 0.25); // 25% from the left
    const targetY = targetBoundingBox.y + targetBoundingBox.height + 10; // 10px below the div
    ...
    try {
      ...
      for (let i = 0; i <= 5; i++) {
        const x = sourceCenter.x + (targetX - sourceCenter.x) * (i / 5);
        const y = sourceCenter.y + (targetY - sourceCenter.y) * (i / 5);
      ...
      // Validate that second video is actually added
      const secondVideoAdded = await page.locator('#expoContentWrapper .sc-gEvEer').filter({ hasText: 'sample_video2.MOV' }).first().isVisible();
      ...
      if (secondVideoAdded) {
        ...
        // Additional checks if video text is not found
        ...
        if (videoCount >= 2) {
          console.log('At least two videos are present in Expo. Assuming successful addition.');
        } else {
